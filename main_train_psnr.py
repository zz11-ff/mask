...
def im2tensor(image, imtype=np.uint8, cent=1., factor=255. / 2.):
    # 灰度图 (H, W)
    if image.ndim == 2:
        image = image[:, :, np.newaxis]  # 变成 (H, W, 1)
...
def main(json_path='options/masked_denoising/input_mask_80_90.json'):
    ...
    # ----------------------------------------
    '''
    total_iter = opt['train'].get('total_iter', 1000000)
    for epoch in range(1000000):  # keep running
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step > total_iter:
                if opt['rank'] == 0:
                    logger.info('Reach total_iter: {:d}. Finish training.'.format(total_iter))
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0:
                ...
                for test_data in test_loader:
                    ...
                    out = (visuals['E'] + 1) / 2  # [-1,1] → [0,1]
                    gt = (visuals['H'] + 1) / 2

                    E_img = util.tensor2uint(out)
                    H_img = util.tensor2uint(gt)
                    # ========== 修复尺寸不一致：强制裁剪模型输出 ==========
                    min_h = min(E_img.shape[0], H_img.shape[0])
                    min_w = min(E_img.shape[1], H_img.shape[1])
                    E_img = E_img[:min_h, :min_w]
                    H_img = H_img[:min_h, :min_w]
                    # =====================================================
