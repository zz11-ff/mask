...
def input_mask(image, prob_=0.75, value=0.1):
    """
    Multiplicative Bernoulli mask for GRAYSCALE image.
    Keeps single-channel format.
    """
    x, y = image.shape[:2]

    # Generate single-channel mask (H, W)
    mask = np.random.choice([0, 1], size=(x, y), p=[prob_, 1 - prob_]).astype(np.float32)

    # Apply mask to grayscale image
    noise_image = image * mask[..., None]   # keep shape (H, W, 1)
    noise_image = noise_image - value + value * mask[..., None]
...
def input_mask_with_noise(img, sf=1, lq_patchsize=64, noise_level=15, if_mask=True, mask1=75, mask2=75):
    ...
    # noise: ensure grayscale noise, not color noise
    if noise_level > 0:
        noise = np.random.normal(0, noise_level/255.0, (h, w, 1)).astype(np.float32)
        img = img + noise
        img = np.clip(img, 0.0, 1.0)
